...
export function createAuthWindow() {
	...
	webRequest.onBeforeRequest(filter, async ({ url }) => {
		try {
			...
			console.log(
				"[AuthWindow] Tokens loaded client-side (Keytar updated)."
			)

			if (tokenData && tokenData.rawRefreshToken) {
				console.log(
					"[AuthWindow] Attempting to store Auth0 refresh token on server..."
				)
				const serverStoreUrl = `${process.env.APP_SERVER_URL}/auth/store_session`
				const serverStorePayload = {
					refresh_token: tokenData.rawRefreshToken
				}

				const currentAccessToken = getAccessToken() // Get the newly acquired access token
				if (!currentAccessToken) {
					console.error(
						"[AuthWindow] Critical: Access token not available after loadTokens for server call."
					)
					...
					try {
						...
						if (!serverResponse.ok) {
							...
							console.error(
								`[AuthWindow] Failed to store refresh token on server: ${serverResponse.status} - ${errorText}`
							)
						} else {
							const serverResult = await serverResponse.json()
							console.log(
								"[AuthWindow] Successfully sent Auth0 refresh token to server:",
								serverResult.message
							)
						}
					} catch (e) {
						console.error(
							"[AuthWindow] Error sending Auth0 refresh token to server:",
							e
						)
					}
				}
				...
				console.warn(
					"[AuthWindow] Did not receive refresh token from loadTokens, cannot store on server."
				)
			}
			...
			console.error(
				"[AuthWindow] Error during token processing or validity check:",
				error
			)
