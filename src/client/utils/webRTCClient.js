...
export class WebRTCClient {
	...
	async connect(deviceId = null) { // Accept deviceId
		try {
			...
			console.log(`[WebRTCClient] Attempting to connect using deviceId: ${deviceId || 'default'}`);
			...
			try {
                ...
                const constraints = { 
                    audio: deviceId ? { deviceId: { exact: deviceId }, sampleRate: 16000, echoCancellation: true, noiseSuppression: true } : { sampleRate: 16000, echoCancellation: true, noiseSuppression: true },
                    video: false 
                };
				this.mediaStream = await navigator.mediaDevices.getUserMedia(constraints)
				console.log("[WebRTCClient] Got user media with constraints:", constraints);
			} catch (mediaError) {
				...
				if (mediaError.name === "NotAllowedError") {
                    ...
                    throw new Error("Selected microphone not found or no microphone detected. Please check your microphone settings and try again.");
                } else if (mediaError.name === "OverconstrainedError"){
					throw new Error(`Could not satisfy microphone constraints (e.g. sample rate). Device: ${deviceId}. Error: ${mediaError.message}`);
				}
				throw mediaError 
			}
			...
			const response = await fetch(this.signalingServerUrl, {
				...
				body: JSON.stringify({
					sdp: offer.sdp,
					type: offer.type,
					webrtc_id: "client_" + Math.random().toString(36).substring(2, 9) 
				})
			})
			...
			await this.peerConnection.setRemoteDescription(new RTCSessionDescription(serverResponse)) 
