...
export class WebSocketAudioClient {
	...
	// This should come from an environment variable (process.env.NEXT_PUBLIC_APP_SERVER_URL)
	// The NEXT_PUBLIC_ prefix is required to expose it to the browser in Next.js
	serverUrl = `${(process.env.NEXT_PUBLIC_APP_SERVER_URL || "http://localhost:5000").replace(/^http/, "ws")}/ws/voice`

	constructor(options = {}) {
		this.options = options
		this.websocketId =
			"ws_client_" + Math.random().toString(36).substring(2, 9)
		console.log(
			`[WebSocketAudioClient] Initialized with ID: ${this.websocketId}. Server URL: ${this.serverUrl}`
		)
	}
	...
	async setupAudioProcessing() {
		...
		try {
			...
			console.log(
				`[WebSocketAudioClient] AudioContext sample rate: ${this.audioContext.sampleRate}`
			)
			...
			try {
				...
				console.error(
					`[WebSocketAudioClient] Failed to load AudioWorklet from ${workletUrl}:`,
					e
				)
				throw new Error(
					`Failed to load AudioWorklet: ${e.message}. Ensure audio-processor.js is in the public directory.`
				)
			}
	...
	setupWebSocketHandlers() {
		...
		this.ws.onopen = () => {
			...
			if (token) {
				...
				console.log(
					"[WebSocketAudioClient] Sent auth message to main server WS."
				)
			} else {
				console.error(
					"[WebSocketAudioClient] No access token found for WS auth."
				)
				this.ws?.close(
					1008,
		...
		this.ws.onmessage = (event) => {
			if (
				...
				if (event.data instanceof ArrayBuffer) {
					...
					console.warn(
						"[WebSocketAudioClient] Received Blob audio, needs async processing."
					)
				...
				try {
					...
					if (data.type === "auth_success") {
						console.log(
							`[WebSocketAudioClient] WebSocket authenticated with server for user: ${data.user_id}.`
						)
						...
						if (
							...
							this.audioContext
								...
								.then(() =>
									console.log(
										"[WebSocketAudioClient] AudioContext resumed."
									)
								)
						}
					} else if (data.type === "auth_failure") {
						console.error(
							`[WebSocketAudioClient] WebSocket authentication failed: ${data.message}`
						)
						this.handleError(
							new Error(`Authentication failed: ${data.message}`)
						)
						this.disconnect() // Disconnect on auth failure
					} else if (
						data.type === "stt_result" &&
						this.options.onTextMessage
					) {
						...
						console.log(
							"[WebSocketAudioClient] Received TTS stream end signal."
						)
					...
					console.error(
						"[WebSocketAudioClient] Error parsing received JSON message:",
						e,
						"Raw data:",
						event.data
					)
	...
	cleanupAudioProcessing() {
		...
		if (this.audioContext && this.audioContext.state !== "closed") {
			this.audioContext
				.close()
				.then(() =>
					console.log("[WebSocketAudioClient] AudioContext closed.")
				)
