...
# STT/TTS Service Configs 
ELEVENLABS_API_KEY = os.getenv("ELEVENLABS_API_KEY") # Used for both STT and TTS from ElevenLabs
# TTS Provider Choice (Production)
TTS_PROVIDER = os.getenv("TTS_PROVIDER", "ELEVENLABS") # Default to ELEVENLABS if not specified for prod
...
OPENROUTER_MODEL_NAME = os.getenv("OPENROUTER_MODEL_NAME", "meta-llama/llama-3.1-8b-instruct") 
...
print(f"[{datetime.datetime.now()}] [MainServer_Config] TTS_PROVIDER (Prod): {TTS_PROVIDER}")
print(f"[{datetime.datetime.now()}] [MainServer_Config] STT (Dev): FasterWhisper, STT (Prod): ElevenLabs")