...
class MainWebSocketManager:
    ...
    async def connect_voice(self, websocket: WebSocket, user_id: str):
        if user_id in self.voice_connections:
            ...
            if old_ws:
                ...
                except Exception as e: print(f"[{datetime.datetime.now()}] [WS_VOICE_MGR_ERROR] Closing old voice WS for {user_id}: {e}")
        self.voice_connections[user_id] = websocket
        print(f"[{datetime.datetime.now()}] [WS_VOICE_MGR] Voice WebSocket connected for user: {user_id}. Total voice: {len(self.voice_connections)}")
    ...
    def disconnect_voice(self, websocket: WebSocket):
        ...
        if uid and uid in self.voice_connections:
            ...
            print(f"[{datetime.datetime.now()}] [WS_VOICE_MGR] Voice WebSocket disconnected for user: {uid}. Total voice: {len(self.voice_connections)}")
    ...
    async def connect_notifications(self, websocket: WebSocket, user_id: str):
        if user_id in self.notification_connections:
            ...
            if old_ws:
                ...
                except Exception as e: print(f"[{datetime.datetime.now()}] [WS_NOTIF_MGR_ERROR] Closing old notif WS for {user_id}: {e}")
        self.notification_connections[user_id] = websocket
        print(f"[{datetime.datetime.now()}] [WS_NOTIF_MGR] Notification WebSocket connected for user: {user_id}. Total notif: {len(self.notification_connections)}")
    ...
    def disconnect_notifications(self, websocket: WebSocket):
        ...
        if uid and uid in self.notification_connections:
            ...
            print(f"[{datetime.datetime.now()}] [WS_NOTIF_MGR] Notification WebSocket disconnected for user: {uid}. Total notif: {len(self.notification_connections)}")
    ...
    async def send_personal_json_message(self, message_data: Dict[str, Any], user_id: str, connection_type: str = "notifications"):
        ...
        if websocket:
            try:
                ...
                print(f"[{datetime.datetime.now()}] [WS_MGR_INFO] WebSocket for user {user_id} ({connection_type}) already disconnected during send.")
                if connection_type == "notifications": self.disconnect_notifications(websocket)
                else: self.disconnect_voice(websocket)
            except Exception as e:
                print(f"[{datetime.datetime.now()}] [WS_MGR_ERROR] Sending JSON to {user_id} ({connection_type}): {e}. Disconnecting.")
                if connection_type == "notifications": self.disconnect_notifications(websocket)
                else: self.disconnect_voice(websocket)
    
    async def broadcast_json_to_all_notifications(self, message_data: Dict[str, Any]):
        disconnected_sockets = []
        for user_id, websocket in list(self.notification_connections.items()): # Iterate over a copy for safe removal
            try:
                await websocket.send_json(message_data)
            except WebSocketDisconnect:
                print(f"[{datetime.datetime.now()}] [WS_MGR_INFO] WebSocket for user {user_id} (notifications) already disconnected during broadcast.")
                disconnected_sockets.append(websocket)
            except Exception as e:
                print(f"[{datetime.datetime.now()}] [WS_MGR_ERROR] Broadcasting notif to {user_id}: {e}. Marking for disconnect.")
                disconnected_sockets.append(websocket)
