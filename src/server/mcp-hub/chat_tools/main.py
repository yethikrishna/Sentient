...
from . import auth, config, db, kafka_clients
...
task_db_manager = db.TaskDBManager()
...
async def create_task_from_description(ctx: Context, task_description: str) -> Dict[str, Any]:
    ...
    try:
        ...
        producer = await kafka_clients.KafkaTaskProducer.get_producer()
        
        event_payload = {
            "user_id": user_id,
            "action_items": [task_description],
        ...
        await producer.send_and_wait(config.ACTION_ITEMS_TOPIC, value=event_payload)
