...
async def get_evernote_creds(user_id: str) -> Dict[str, str]:
    ...
    evernote_data = user_doc["userData"].get("integrations", {}).get("evernote")

    if not evernote_data or not evernote_data.get("connected") or "credentials" not in evernote_data:
        raise ToolError(f"Evernote integration not connected or credentials missing for {user_id}.")

    try:
        decrypted_creds_str = aes_decrypt(evernote_data["credentials"])
