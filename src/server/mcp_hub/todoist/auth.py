# src/server/mcp_hub/todoist/auth.py
import os
import json
import base64
...
async def get_todoist_token(user_id: str) -> str:
    """Fetches Todoist access token from MongoDB."""
    user_doc = await users_collection.find_one({"user_id": user_id})

    if not user_doc or not user_doc.get("userData"):
        raise ToolError(f"User profile not found for user_id: {user_id}.")

    todoist_data = user_doc["userData"].get("integrations", {}).get("todoist")

    if not todoist_data or not todoist_data.get("connected") or "credentials" not in todoist_data:
        raise ToolError(f"Todoist integration not connected or credentials missing for {user_id}.")
    ...
    if not access_token:
        raise ToolError("Invalid token data in database for Todoist. 'access_token' not found.")
